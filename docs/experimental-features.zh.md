...
- [客户端模式 DHT 路由](#%E5%AE%A2%E6%88%B7%E7%AB%AF%E6%A8%A1%E5%BC%8F-dht-%E8%B7%AF%E7%94%B1)
- [go-multiplex stream muxer](#go-multiplex-stream-muxer)
- [原始 unix fs 文件](#%E5%8E%9F%E5%A7%8B-unix-fs-%E6%96%87%E4%BB%B6)
- [ipfs 文件存储](#ipfs-%E6%96%87%E4%BB%B6%E5%AD%98%E5%82%A8)
...
- [Badger 数据存储区](#badger-%E6%95%B0%E6%8D%AE%E5%AD%98%E5%82%A8%E5%8C%BA)
...
要检查在 任何两个给定对等体之间 使用哪个流复用器,请检查该对应的 json 输出`ipfs swarm peers`命令,你会看到这样的事情:
...
然后使用`ipfs urlstore add <url>`，添加一个指定的的 URL 文件。
...
使用生成 预共享密钥[ipfs 群 密钥 生成 ](https://github.com/Kubuxu/go-ipfs-swarm-key-gen)) :
...
- 在一个节点上打开一个侦听器 (节点 A) `ipfs p2p listener open p2p-test /ip4/127.0.0.1/tcp/10101`
- 把你希望传递 p2p 连接的应用程序的地址放到`/ip4/127.0.0.1/tcp/10101`
- 在另一个节点上,连接到 节点 A 上的侦听器`ipfs p2p stream dial $NODE_A_PEERID p2p-test /ip4/127.0.0.1/tcp/10102`
...
Badger-ds 是基于的新数据存储实现<https://github.com/dgraph-io/badger>
...
- [ ] 需要更多人使用并报告其工作情况
- [ ] 为订阅,添加 **最后一次记录**分发的机制,这样我们就不必为了初始解析而点击 DHT. 或者,我们可以定期重新发布最后一条记录.
...
为了 监听 一个 QUIC 地址, 要添加像`/ip4/0.0.0.0/udp/4001/quic`这样的 swarm 地址。
